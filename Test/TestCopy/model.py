...
class CopyState:
    ...
    def move_cursor(self, dx: int = 0, dy: int = 0) -> bool:
        ...
        self.cursor_col = _clamp(self.cursor_col + dx, 0, self._line_max_col(self.cursor_line_abs))
    ...
    def move_end(self) -> bool:
        if self.mode not in {"copy", "search"}:
            return False
        new_col = self._line_max_col(self.cursor_line_abs)
    ...
    def _page_move(self, direction: int) -> bool:
        ...
        self.cursor_col = min(self.cursor_col, self._line_max_col(self.cursor_line_abs))
        self._sync_selection_to_cursor()
        return True
    ...
    def _line_max_col(self, line_abs: int) -> int:
        if not self.lines:
            return 0
